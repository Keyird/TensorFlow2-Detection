...
from networks.loss import yolo_loss
from networks.yolov4 import yolo_body
...
if __name__ == "__main__":
    ...
    annotation_path = 'data/2007_train.txt'  # 获得图片路径和标签
    log_dir = 'logs/'  # 训练后的模型保存的位置
    classes_path = 'data/voc_classes.txt'  # 训练前一定要修改classes_path，使其对应自己的数据集
    anchors_path = 'data/yolo_anchors.txt'  # 加载anchor的路径
    # 训练自己的数据集时提示维度不匹配正常，预测的东西都不一样了自然维度不匹配
    weights_path = 'data/yolov4_voc_weights.h5'
    # 训练用图片大小，一般在416x416和608x608选择
    input_shape = (416, 416)
    # 是否对损失进行归一化，用于改变loss的大小：用于决定计算最终loss是除上batch_size还是除上正样本数量
    normalize = False
    class_names = get_classes(classes_path)  # 获取classes
    anchors = get_anchors(anchors_path)  # 获取anchor
    num_classes = len(class_names)
    num_anchors = len(anchors)  # 一共有多少先验框
