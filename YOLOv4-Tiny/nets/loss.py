...
from nets.yolov4_tiny import yolo_head
import math
...
def yolo_loss(args, anchors, num_classes, ignore_thresh=.5, label_smoothing=0.1, print_loss=False, normalize=True):
    ...
    # args是[*model_body.output, *y_true]
    y_true = args[num_layers:]  # y_true是一个列表，包含两个特征层，shape分别为(m,13,13,3,85),(m,26,26,3,85)
    yolo_outputs = args[:num_layers]  # yolo_outputs是一个列表，包含两个特征层，shape分别为(m,13,13,3,85),(m,26,26,3,85)
    ...
    # 取出每一张图片，m的值就是batch_size
    m = K.shape(yolo_outputs[0])[0]
    mf = K.cast(m, K.dtype(yolo_outputs[0]))

    for l in range(num_layers):
        # 以第一个特征层(m,13,13,3,85)为例子，取出该特征层中存在目标的点的位置(m,13,13,3,1)
        object_mask = y_true[l][..., 4:5]
        # 取出其对应的种类(m,13,13,3,80)
        true_class_probs = y_true[l][..., 5:]

        if label_smoothing:
            true_class_probs = _smooth_labels(true_class_probs, label_smoothing)   # 标签平滑

        #   将yolo_outputs的特征层输出进行解析、获得四个返回值，其中：
        #   grid        (13,13,1,2) 网格坐标
        ...
        # pred_box是解码后的预测的box的真实位置[x,y,w,h]，size为(m,13,13,3,4)
        ...
        #   该操作的目的是：忽略预测结果与真实框非常对应特征点，因为这些框已经比较准了，不适合当作负样本，所以忽略掉。
        # ------------------------------------------------------------------------------#
