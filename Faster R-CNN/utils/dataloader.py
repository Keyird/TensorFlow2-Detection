...
class FRCNNDatasets():
    ...
    def generate(self):
        ...
        while True:
            ...
            for b in range(self.batch_size):
                if i == 0:
                    np.random.shuffle(self.annotation_lines)  # 随机打乱
    ...
    def encode_ignore_box(self, box, return_iou=True, variances=[0.25, 0.25, 0.25, 0.25]):
        ...
        # 逆向编码，将真实框转化为FRCNN预测结果的格式, 先计算真实框的中心与长宽
    ...
    def assign_boxes(self, boxes):
        ...
        # 在reshape后，获得的ingored_boxes的shape为：[num_true_box, num_anchors, 1] 其中1为iou
        ingored_boxes = ingored_boxes.reshape(-1, self.num_anchors, 1)
        ignore_iou = ingored_boxes[:, :, 0].max(axis=0)
        ignore_iou_mask = ignore_iou > 0
        assignment[:, 4][ignore_iou_mask] = -1

        # 在reshape后，获得的encoded_boxes的shape为：[num_true_box, num_anchors, 4+1], 4是编码后的结果，1为iou
